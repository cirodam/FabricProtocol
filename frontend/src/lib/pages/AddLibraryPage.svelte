<script lang="ts">
  ...
  async function submit(e: Event) {
    ...
    try {
      ...
      const res = await fetch("/api/education/libraries", {
        method: "POST",
      ...
      navigate(`/education/libraries/${created.id}`);
    } catch (e: unknown) {
...
<div class="page">
  ...
  <form class="form" onsubmit={submit}>
    ...
    <label>
      ...
      <input type="text" bind:value={name} placeholder="e.g. Lincoln Neighborhood Library" disabled={working} required />
    </label>
    ...
    <div class="actions">
      <button type="submit" class="primary-btn" disabled={working || !name.trim()}>
        {working ? "Creating…" : "Create library"}
      </button>
      <button type="button" class="secondary-btn" onclick={() => navigate("/education")} disabled={working}>
        Cancel
      </button>
    </div>
  </form>
