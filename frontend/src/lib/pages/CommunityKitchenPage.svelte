<script lang="ts">
  ...
  let kitchen     = $state<KitchenDetail | null>(null);
  ...
  let availableMembers = $derived(
    allMembers.filter(m => !kitchen?.staff.some(s => s.id === m.id))
  );

  async function load() {
    try {
      const [kitchenRes, membersRes] = await Promise.all([
        fetch(`/api/food/kitchens/${id}`),
        fetch(`/api/members`),
      ]);
      if (!kitchenRes.ok) throw new Error(`HTTP ${kitchenRes.status}`);
      if (!membersRes.ok) throw new Error(`HTTP ${membersRes.status}`);
      kitchen    = await kitchenRes.json();
      allMembers = await membersRes.json();
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to load kitchen";
    } finally {
      loading = false;
    }
  }
  ...
  async function addStaff(e: Event) {
    ...
    addError = "";
    try {
      const res = await fetch(`/api/food/kitchens/${id}/staff`, {
        method: "POST",
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/food/kitchens/${id}/staff/${memberId}`, { method: "DELETE" });
  ...
  async function deleteKitchen() {
    try {
      const res = await fetch(`/api/food/kitchens/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      navigate("/food");
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to delete kitchen";
    }
  }
...
<div class="page">
  ...
  {:else if kitchen}
    <div class="header">
      <div>
        <div class="badge">Community Kitchen</div>
        <h1>{kitchen.name}</h1>
        <p class="desc">{kitchen.description}</p>
      </div>
    ...
    {#if confirmDelete}
      <div class="confirm-box">
        <p>Delete <strong>{kitchen.name}</strong>? This cannot be undone.</p>
        <div class="confirm-actions">
          <button onclick={() => confirmDelete = false}>Cancel</button>
          <button class="danger-btn" onclick={deleteKitchen}>Confirm delete</button>
        </div>
      </div>
    {/if}

    <section class="section">
      <h2>Staff ({kitchen.staffCount})</h2>

      {#if kitchen.staff.length === 0}
      ...
      <form class="add-form" onsubmit={addStaff}>
        ...
        <div class="add-grid">
          ...
          <input type="text" bind:value={addTitle} placeholder="Role title (e.g. Cook)" disabled={addWorking} />
