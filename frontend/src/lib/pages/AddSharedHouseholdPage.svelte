<script lang="ts">
  ...
  async function submit(e: Event) {
    ...
    error = "";
    try {
      const res = await fetch("/api/dependency-care/households", {
        method: "POST",
      ...
      navigate(`/dependency-care/households/${household.id}`);
    } catch (e: unknown) {
...
<div class="page">
  ...
  <form onsubmit={submit} class="form">
    ...
    <label>
      ...
      <input type="text" bind:value={name} placeholder="e.g. Elm Street House" required />
    </label>
    ...
    <div class="actions">
      ...
      <button type="submit" class="submit-btn" disabled={submitting}>{submitting ? "Creating…" : "Create Household"}</button>
    </div>
  </form>
</div>
