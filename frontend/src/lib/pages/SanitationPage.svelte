<script lang="ts">
  ...
  async function assign(roleId: string) {
    assignError = "";
    const res = await fetch(`/api/sanitation/roles/${roleId}/assign`, {
      method: "POST",
  ...
  async function unassign(roleId: string) {
    await fetch(`/api/sanitation/roles/${roleId}/assign`, { method: "DELETE" });
    await load();
  }
...
<div class="domain-main">
  <div class="page-header">
    <div>
      <h1>Sanitation</h1>
      <p class="subtitle">Waste management, hygiene infrastructure, and disease prevention</p>
    </div>
  </div>


  <DomainPoolPanel domainId="00000000-0000-0000-0000-000000000008" {navigate} />
