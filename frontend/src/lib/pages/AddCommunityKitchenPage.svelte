<script lang="ts">
  ...
  async function submit(e: Event) {
    ...
    try {
      ...
      const res = await fetch("/api/food/kitchens", {
        method: "POST",
      ...
      navigate(`/food/kitchens/${created.id}`);
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to create kitchen";
      working = false;
    }
  }
</script>
...
<div class="page">
  ...
  <form class="form" onsubmit={submit}>
    ...
    <label>
      ...
      <input type="text" bind:value={name} placeholder="e.g. East Side Kitchen" disabled={working} required />
    </label>
    ...
    <div class="actions">
      <button type="submit" class="primary-btn" disabled={working || !name.trim()}>
        {working ? "Creating…" : "Create kitchen"}
      </button>
