<script lang="ts">
  ...
  let household     = $state<HouseholdDetail | null>(null);
  ...
  let availableMembers = $derived(
    allMembers.filter(m => !household?.staff.some(s => s.id === m.id))
  );

  async function load() {
    try {
      const [hRes, membersRes] = await Promise.all([
        fetch(`/api/dependency-care/households/${id}`),
        fetch(`/api/members`),
      ]);
      if (!hRes.ok) throw new Error(`HTTP ${hRes.status}`);
      if (!membersRes.ok) throw new Error(`HTTP ${membersRes.status}`);
      household  = await hRes.json();
      allMembers = await membersRes.json();
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to load household";
    } finally {
      loading = false;
    }
  }
  ...
  async function addStaff(e: Event) {
    ...
    addError = "";
    try {
      const res = await fetch(`/api/dependency-care/households/${id}/staff`, {
        method: "POST",
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/dependency-care/households/${id}/staff/${memberId}`, { method: "DELETE" });
  ...
  async function deleteHousehold() {
    try {
      const res = await fetch(`/api/dependency-care/households/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      navigate("/dependency-care");
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to delete household";
    }
  }
...
<div class="page">
  ...
  {:else if household}
    <div class="header">
      <div>
        <div class="badge">Shared Household</div>
        <h1>{household.name}</h1>
        {#if household.description}<p class="desc">{household.description}</p>{/if}
      </div>
    ...
    {#if confirmDelete}
      <div class="confirm-box">
        <p>Delete <strong>{household.name}</strong>? This cannot be undone.</p>
        <div class="confirm-actions">
          <button onclick={() => confirmDelete = false}>Cancel</button>
          <button class="danger-btn" onclick={deleteHousehold}>Confirm delete</button>
        </div>
      </div>
    {/if}

    <section class="section">
      <h2>Staff ({household.staffCount})</h2>

      {#if household.staff.length === 0}
      ...
      {:else}
        <table class="staff-table">
          ...
          <tbody>
            {#each household.staff as s (s.id)}
              <tr>
                <td>
      ...
      <form class="add-form" onsubmit={addStaff}>
        ...
        <div class="add-grid">
          ...
          <input type="text" bind:value={addTitle} placeholder="Role title (e.g. Care Worker)" disabled={addWorking} />
...
<style>
  ...
  .badge {
    ...
    background: var(--color-primary-light, #dbeafe); color: var(--color-primary, #1d4ed8);
