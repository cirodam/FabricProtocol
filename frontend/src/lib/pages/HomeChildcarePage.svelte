<script lang="ts">
  ...
  let unit        = $state<HcDetail | null>(null);
  ...
  async function addStaff(e: Event) {
    ...
    addError = "";
    try {
      const res = await fetch("/api/child-care/home-childcare/staff", {
        method: "POST",
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/child-care/home-childcare/staff/${memberId}`, { method: "DELETE" });
...
<div class="page">
  <button class="back-link" onclick={() => navigate("/child-care")}>← Child Care</button>
  ...
  {:else if unit}
    <div class="header">
      <div>
        <div class="badge">Home Childcare</div>
        <h1>{unit.name}</h1>
    ...
    <section class="section">
      ...
      <form class="add-form" onsubmit={addStaff}>
        ...
        <div class="add-grid">
          ...
          <input type="text" bind:value={addTitle} placeholder="Role title (e.g. Home Childcarer)" disabled={addWorking} />
