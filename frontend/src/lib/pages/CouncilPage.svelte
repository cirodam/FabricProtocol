<script lang="ts">
  const { id: domainId, navigate }: { id: string; navigate: (path: string) => void } = $props();
...
<div class="page">
  ...
  {:else if council}
    <div class="header">
      <div>
        <span class="badge">Domain Council</span>
        <h1>{council.domainName}</h1>
        <p class="meta">{council.seatCount} / {council.size} seats filled</p>
      </div>
      {#if council.vacancies > 0 && council.poolId}
        <button class="primary draw-btn" onclick={draw} disabled={drawWorking}>
    ...
    <section class="section">
      ...
      {#if council.seats.length === 0}
        <p class="muted">
          {council.poolId ? 'No seats filled yet. Use Draw to select members.' : 'Link a sortition pool below, then draw seats.'}
        </p>
      {:else}
...
<style>
  ...
  .badge { display: inline-block; padding: 0.2rem 0.6rem; border-radius: 999px; font-size: 0.75rem; font-weight: 600; background: #fef9c3; color: #854d0e; margin-bottom: 0.4rem; }
  h1 { margin: 0 0 0.2rem; font-size: 1.4rem; font-weight: 600; }
  .meta { margin: 0; font-size: 0.85rem; color: var(--text-secondary); }
  .draw-btn { align-self: flex-start; }
  ...
  .vacate-btn { background: none; border: 1px solid var(--color-border, #e2e8f0); border-radius: 4px; padding: 0.2rem 0.6rem; font-size: 0.8rem; cursor: pointer; color: var(--color-danger, #dc2626); }

  .pool-row { display: flex; gap: 0.5rem; align-items: center; }
  .pool-row select { flex: 1; padding: 0.5rem 0.75rem; border: 1px solid var(--color-border, #e2e8f0); border-radius: 6px; font-size: 0.9rem; background: var(--color-surface, #fff); }
