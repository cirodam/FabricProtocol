<script lang="ts">
  ...
  let clinic       = $state<DentalClinicDetail | null>(null);
  ...
  async function load() {
    try {
      const [clinicRes, membersRes] = await Promise.all([
        fetch(`/api/healthcare/dental-clinics/${id}`),
        fetch(`/api/members`),
      ]);
      ...
      error = e instanceof Error ? e.message : "Failed to load dental clinic";
    } finally {
      loading = false;
    }
  }
  ...
  async function addStaff(e: Event) {
    ...
    addError   = "";
    try {
      const res = await fetch(`/api/healthcare/dental-clinics/${id}/staff`, {
        method: "POST",
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/healthcare/dental-clinics/${id}/staff/${memberId}`, { method: "DELETE" });
  ...
  async function deleteClinic() {
    try {
      const res = await fetch(`/api/healthcare/dental-clinics/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      navigate("/healthcare");
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to delete dental clinic";
    }
  }
...
<div class="page">
  ...
  {:else if clinic}
    <div class="header">
      <div>
        <div class="badge">Dental Clinic</div>
        <h1>{clinic.name}</h1>
    ...
    <section class="section">
      ...
      <form class="add-form" onsubmit={addStaff}>
        ...
        <div class="add-grid">
          ...
          <input type="text" bind:value={addTitle} placeholder="Role title (e.g. Dentist)" disabled={addWorking} />
