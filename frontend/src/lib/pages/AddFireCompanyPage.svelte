<script lang="ts">
  ...
  async function submit(e: Event) {
    ...
    try {
      ...
      navigate(`/fire/companies/${data.id}`);
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to create fire company";
      working = false;
    }
  }
</script>
...
<div class="page">
  ...
  <form onsubmit={submit} class="form">
    ...
    <label>
      ...
      <input type="text" bind:value={name} placeholder="e.g. Station 1 – East District" disabled={working} />
    </label>

    <label>
      <span>Description</span>
      <textarea bind:value={description} rows="3" placeholder="Response zone, apparatus, notes…" disabled={working}></textarea>
    </label>

    <div class="actions">
      <button type="button" onclick={() => navigate("/fire")} disabled={working}>Cancel</button>
      <button type="submit" class="primary" disabled={working || !name.trim()}>{working ? "Creating…" : "Create fire company"}</button>
    </div>
  </form>
</div>
...
<style>
  ...
  input, textarea { padding: 0.5rem 0.75rem; border: 1px solid var(--color-border, #e2e8f0); border-radius: 6px; font-size: 0.9rem; background: var(--color-surface, #fff); font-family: inherit; resize: vertical; }
