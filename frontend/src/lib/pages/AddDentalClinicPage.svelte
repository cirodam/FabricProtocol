<script lang="ts">
  ...
  async function submit(e: Event) {
    ...
    try {
      ...
      const res = await fetch("/api/healthcare/dental-clinics", {
        method: "POST",
      ...
      navigate(`/healthcare/dental-clinics/${created.id}`);
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to create dental clinic";
      working = false;
    }
  }
</script>
...
<div class="page">
  ...
  <form class="form" onsubmit={submit}>
    ...
    <label>
      ...
      <input type="text" bind:value={name} placeholder="e.g. North Side Dental Clinic" disabled={working} required />
    </label>
    ...
    <div class="actions">
      <button type="submit" class="primary-btn" disabled={working || !name.trim()}>
        {working ? "Creating…" : "Create dental clinic"}
      </button>
      <button type="button" class="secondary-btn" onclick={() => navigate("/healthcare")} disabled={working}>
        Cancel
      </button>
    </div>
  </form>
