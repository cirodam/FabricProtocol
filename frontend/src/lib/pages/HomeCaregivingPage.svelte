<script lang="ts">
  ...
  let unit         = $state<HcgDetail | null>(null);
  ...
  async function addStaff(e: Event) {
    ...
    addError = "";
    try {
      const res = await fetch("/api/dependency-care/home-caregiving/staff", {
        method: "POST",
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/dependency-care/home-caregiving/staff/${memberId}`, { method: "DELETE" });
...
<div class="page">
  ...
  {:else if unit}
    <div class="header">
      <div>
        <div class="badge">Home Caregiving</div>
        <h1>{unit.name}</h1>
    ...
    <section class="section">
      ...
      <form class="add-form" onsubmit={addStaff}>
        ...
        <div class="add-grid">
          ...
          <input type="text" bind:value={addTitle} placeholder="Role title (e.g. Home Caregiver)" disabled={addWorking} />
