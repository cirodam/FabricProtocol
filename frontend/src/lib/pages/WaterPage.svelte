<script lang="ts">
  ...
  async function assign(roleId: string) {
    assignError = "";
    const res = await fetch(`/api/water/roles/${roleId}/assign`, {
      method: "POST",
  ...
  async function unassign(roleId: string) {
    await fetch(`/api/water/roles/${roleId}/assign`, { method: "DELETE" });
    await load();
  }
...
<div class="domain-main">
  <div class="page-header">
    <div>
      <h1>Water</h1>
      <p class="subtitle">Freshwater extraction, treatment, storage, distribution, and irrigation</p>
    </div>
  </div>


  <DomainPoolPanel domainId="00000000-0000-0000-0000-000000000009" {navigate} />
