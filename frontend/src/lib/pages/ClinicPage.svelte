<script lang="ts">
  ...
  let clinic      = $state<ClinicDetail | null>(null);
  ...
  let availableMembers = $derived(
    allMembers.filter(m => !clinic?.staff.some(s => s.id === m.id))
  );

  async function load() {
    try {
      const [clinicRes, membersRes] = await Promise.all([
        fetch(`/api/healthcare/clinics/${id}`),
        fetch(`/api/members`),
      ]);
      if (!clinicRes.ok) throw new Error(`HTTP ${clinicRes.status}`);
      if (!membersRes.ok) throw new Error(`HTTP ${membersRes.status}`);
      clinic     = await clinicRes.json();
      allMembers = await membersRes.json();
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to load clinic";
    } finally {
      loading = false;
    }
  }
  ...
  async function addStaff(e: Event) {
    ...
    addError = "";
    try {
      const res = await fetch(`/api/healthcare/clinics/${id}/staff`, {
        method: "POST",
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/healthcare/clinics/${id}/staff/${memberId}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      await load();
    } catch {
      // silently reload; worst case staff list is stale
    }
  }

  async function deleteClinic() {
    try {
      const res = await fetch(`/api/healthcare/clinics/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      navigate("/healthcare"); // delete → back to list
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to delete clinic";
    }
  }
...
<div class="page">
  ...
  {:else if clinic}
    <div class="header">
      <div>
        <div class="badge">Primary Care</div>
        <h1>{clinic.name}</h1>
        <p class="desc">{clinic.description}</p>
      </div>
    ...
    {#if confirmDelete}
      <div class="confirm-box">
        <p>Delete <strong>{clinic.name}</strong>? This cannot be undone.</p>
        <div class="confirm-actions">
          <button onclick={() => confirmDelete = false}>Cancel</button>
          <button class="danger-btn" onclick={deleteClinic}>Confirm delete</button>
        </div>
      </div>
    {/if}

    <section class="section">
      <h2>Staff ({clinic.staffCount})</h2>
      ...
      <form class="add-form" onsubmit={addStaff}>
        ...
        <div class="add-grid">
          ...
          <input type="text" bind:value={addTitle} placeholder="Role title (e.g. Physician)" disabled={addWorking} />
