<script lang="ts">
  ...
  let availableMembers = $derived(
    allMembers.filter(m => !unit?.staff.some(s => s.id === m.id))
  );

  async function load() {
    try {
      const [uRes, membersRes] = await Promise.all([
        fetch(`/api/dependency-care/medical-care-units/${id}`),
        fetch(`/api/members`),
      ]);
      if (!uRes.ok) throw new Error(`HTTP ${uRes.status}`);
      if (!membersRes.ok) throw new Error(`HTTP ${membersRes.status}`);
      unit       = await uRes.json();
  ...
  async function addStaff(e: Event) {
    ...
    addError = "";
    try {
      const res = await fetch(`/api/dependency-care/medical-care-units/${id}/staff`, {
        method: "POST",
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/dependency-care/medical-care-units/${id}/staff/${memberId}`, { method: "DELETE" });
  ...
  async function deleteUnit() {
    try {
      const res = await fetch(`/api/dependency-care/medical-care-units/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      navigate("/dependency-care");
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to delete unit";
    }
  }
...
<div class="page">
  ...
  {:else if unit}
    <div class="header">
      <div>
        <div class="badge">Medical Care Unit</div>
        <h1>{unit.name}</h1>
        {#if unit.description}<p class="desc">{unit.description}</p>{/if}
      </div>
    ...
    {#if confirmDelete}
      <div class="confirm-box">
        <p>Delete <strong>{unit.name}</strong>? This cannot be undone.</p>
        <div class="confirm-actions">
          <button onclick={() => confirmDelete = false}>Cancel</button>
          <button class="danger-btn" onclick={deleteUnit}>Confirm delete</button>
        </div>
      </div>
    {/if}

    <section class="section">
      <h2>Staff ({unit.staffCount})</h2>
      ...
      <form class="add-form" onsubmit={addStaff}>
        ...
        <div class="add-grid">
          ...
          <input type="text" bind:value={addTitle} placeholder="Role title (e.g. Care Nurse)" disabled={addWorking} />
