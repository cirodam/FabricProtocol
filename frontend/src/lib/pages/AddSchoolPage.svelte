<script lang="ts">
  ...
  async function submit(e: Event) {
    ...
    try {
      ...
      const res = await fetch("/api/education/schools", {
        method: "POST",
      ...
      navigate(`/education/schools/${created.id}`);
    } catch (e: unknown) {
...
<div class="page">
  ...
  <form class="form" onsubmit={submit}>
    ...
    <label>
      ...
      <input type="text" bind:value={name} placeholder="e.g. Lincoln Neighborhood School" disabled={working} required />
    </label>
    ...
    <div class="actions">
      <button type="submit" class="primary-btn" disabled={working || !name.trim()}>
        {working ? "Creating…" : "Create school"}
      </button>
