<script lang="ts">
  ...
  let mill        = $state<MillDetail | null>(null);
  ...
  let availableMembers = $derived(
    allMembers.filter(m => !mill?.staff.some(s => s.id === m.id))
  );

  async function load() {
    try {
      const [millRes, membersRes] = await Promise.all([
        fetch(`/api/food/mills/${id}`),
        fetch(`/api/members`),
      ]);
      if (!millRes.ok) throw new Error(`HTTP ${millRes.status}`);
      if (!membersRes.ok) throw new Error(`HTTP ${membersRes.status}`);
      mill       = await millRes.json();
      allMembers = await membersRes.json();
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to load mill";
    } finally {
      loading = false;
    }
  }
  ...
  async function addStaff(e: Event) {
    ...
    addError = "";
    try {
      const res = await fetch(`/api/food/mills/${id}/staff`, {
        method: "POST",
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/food/mills/${id}/staff/${memberId}`, { method: "DELETE" });
  ...
  async function deleteMill() {
    try {
      const res = await fetch(`/api/food/mills/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      navigate("/food");
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to delete mill";
    }
  }
...
<div class="page">
  ...
  {:else if mill}
    <div class="header">
      <div>
        <div class="badge">Grain Mill</div>
        <h1>{mill.name}</h1>
        <p class="desc">{mill.description}</p>
      </div>
    ...
    {#if confirmDelete}
      <div class="confirm-box">
        <p>Delete <strong>{mill.name}</strong>? This cannot be undone.</p>
        <div class="confirm-actions">
          <button onclick={() => confirmDelete = false}>Cancel</button>
          <button class="danger-btn" onclick={deleteMill}>Confirm delete</button>
        </div>
      </div>
    {/if}

    <section class="section">
      <h2>Staff ({mill.staffCount})</h2>

      {#if mill.staff.length === 0}
      ...
      <form class="add-form" onsubmit={addStaff}>
        ...
        <div class="add-grid">
          ...
          <input type="text" bind:value={addTitle} placeholder="Role title (e.g. Miller)" disabled={addWorking} />
