<script lang="ts">
  ...
  let availableMembers = $derived(
    allMembers.filter(m => !school?.staff.some(s => s.id === m.id))
  );

  async function load() {
    try {
      const [schoolRes, membersRes] = await Promise.all([
        fetch(`/api/education/schools/${id}`),
        fetch(`/api/members`),
      ]);
  ...
  async function removeStaff(memberId: string) {
    try {
      const res = await fetch(`/api/education/schools/${id}/staff/${memberId}`, { method: "DELETE" });
  ...
  async function deleteSchool() {
    try {
      const res = await fetch(`/api/education/schools/${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      navigate("/education");
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : "Failed to delete school";
    }
  }
  ...
  {:else if school}
    ...
    {#if confirmDelete}
      <div class="confirm-box">
        <p>Delete <strong>{school.name}</strong>? This cannot be undone.</p>
        <div class="confirm-actions">
          <button onclick={() => confirmDelete = false}>Cancel</button>
          <button class="danger-btn" onclick={deleteSchool}>Confirm delete</button>
        </div>
      </div>
    {/if}

    <section class="section">
      <h2>Staff ({school.staffCount})</h2>
