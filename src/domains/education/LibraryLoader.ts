...
export class LibraryLoader {
    ...
    save(library: Library): void {
        const roles: RoleRecord[] = library.getRoles().map(r => ({
            id:              r.id,
            title:           r.title,
        ...
        const record: LibraryRecord = {
            ...
            staffIds:    library.getMembers(),
            roles,
            createdAt:   library.createdAt.toISOString(),
        };
        this.store.write(library.id, record);
    }

    loadAll(): Library[] {
        return this.store.readAll<LibraryRecord>().map(r => this.fromRecord(r));
    }
