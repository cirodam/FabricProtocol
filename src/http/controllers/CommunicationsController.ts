...
// GET /communications/roles
export function listRoles(_req: Request, res: Response): void {
    res.json(CommunicationsDomain.getInstance().getRoles().map(roleToDto));
}

// POST /communications/roles  — body: { title, description?, kinPerMonth? }
export function createRole(req: Request, res: Response): void {
    ...
    CommunicationsDomain.getInstance().addRole(role);
    res.status(201).json(roleToDto(role));
}

// DELETE /communications/roles/:id
export function removeRole(req: Request, res: Response): void {
    const domain = CommunicationsDomain.getInstance();
...
// POST /communications/roles/:id/assign  — body: { memberId }
...
// DELETE /communications/roles/:id/assign  — unassign member from role
