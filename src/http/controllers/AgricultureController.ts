...
// GET /agriculture/roles
export function listRoles(_req: Request, res: Response): void {
    res.json(AgricultureDomain.getInstance().getRoles().map(roleToDto));
}

// POST /agriculture/roles  — body: { title, description?, kinPerMonth? }
export function createRole(req: Request, res: Response): void {
    ...
    AgricultureDomain.getInstance().addRole(role);
    res.status(201).json(roleToDto(role));
}

// DELETE /agriculture/roles/:id
export function removeRole(req: Request, res: Response): void {
    const domain = AgricultureDomain.getInstance();
...
// POST /agriculture/roles/:id/assign  — body: { memberId }
...
// DELETE /agriculture/roles/:id/assign  — unassign member from role
