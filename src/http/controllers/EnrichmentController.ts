...
export function listRoles(_req: Request, res: Response): void {
    res.json(EnrichmentDomain.getInstance().getRoles().map(roleToDto));
}
...
export function listUpcomingEvents(_req: Request, res: Response): void {
    res.json(EnrichmentDomain.getInstance().getUpcoming());
}
