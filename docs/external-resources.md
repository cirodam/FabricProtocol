...
When the community accepts dollars, it is accepting undifferentiated potential that governance then decides how to deploy. The conversion from dollars to credits is a deliberate community act, not an automatic market mechanism.
...
Think of dollars as stem cells: undifferentiated potential. The community takes them in and decides what to grow from them — credits, food, tools, space, care. The conversion is a governance act. The dollar itself is inert until the community gives it direction.
...
- Governance approves the contribution and decides how many credits to issue in return
- Credits are issued via `CentralBank.issueEndowment()` or a one-time Commons transfer — the rate is set by the proposal, not the system
...
- Members who cover expenses are reimbursed in credits per the governance-approved rate

### Off-ramp
- Not supported by default — credits are not redeemable for dollars
...
- GovernanceService gets `approveContribution(memberId, amountUsd, creditsIssued, proposalId)` — increases asset, issues credits
- GovernanceService gets `recordExpensePaid(expenseId, memberId, proposalId)` — decreases asset, issues reimbursement credits
